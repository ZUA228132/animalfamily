...
export default function Home() {
  ...
  // Fetch the latest published announcements from Supabase on mount.
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    // Сохраняем безопасно telegram_id, чтобы TS не ругался
  ...
  async function handleSaveCity(e: FormEvent) {
    ...
    if (!user?.id) {
      alert(
        'Мы не смогли получить ваш Telegram аккаунт. Откройте мини-приложение внутри Telegram.'
      );
      return;
    }
      ...
      <div className="container">
        ...
        {user?.id && !profileCity && (
          <div className="card city-card fade-in">
            ...
            <p>
              Укажите свой город — мы закрепим его за вашим профилем. Это
              поможет лучше настраивать объявления.
            </p>
        ...
        {user?.id && profileCity && (
          <p className="profile-city">
            Ваш город: <strong>{profileCity}</strong>
          </p>
        )}

        {/* Баннер (пока статичный) */}
        <Banner
        ...
        {/* Блок-призыв создать объявление */}
        <div className="card premium-info fade-in">
          <h3>
            {user?.first_name
              ? `${user.first_name}, создайте своё объявление!`
              : 'Создайте своё объявление!'}
          </h3>
          <p>
            Добавьте информацию о потерянном или найденном питомце и помогите
            ему найти дом.
          </p>
        ...
        {announcements.length === 0 && <p>Пока объявлений нет. Будьте первым!</p>}
